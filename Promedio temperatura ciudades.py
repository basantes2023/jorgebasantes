...
ciudades_indices = [0, 1, 2]

for ciudad_index in ciudades_indices:
    print(f'Ciudad {ciudad_index + 1}:')
    for semana_index, semana in enumerate(temperaturas[ciudad_index], start=1):
        suma = 0
        for dia in semana:
            suma += dia['temp']
        promedio_semana = suma / len(semana)
        print(f'Semana {semana_index}: Promedio de temperaturas = {promedio_semana:.2f} grados Celsius')